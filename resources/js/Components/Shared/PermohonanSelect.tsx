...
type Props = {
    className?: string,
    value?: Permohonan | undefined,
    errors?: string,
    onValueChange: (e: Permohonan, opt: OptionSelect) => void,
    inputRef?: any,
    isStaf?: boolean,
}
export default function PermohonanSelect({ className, onValueChange, value, errors, inputRef, isStaf }: Props) {
    const [selectedPerson, setSelectedPerson] = useState<Permohonan | undefined>(value ? value : undefined,);
    ...
    async function fetcher(url: string, query: string | unknown): Promise<Permohonan[]> {
        if (searchKey) {
            const result = await apputils.backend.get(`${url}?search=${query}&search_key=${searchKey}&is_staf=${isStaf}`);
            return result.data
        } else {
            const result = await apputils.backend.get(`${url}?search=${query}&is_staf=${isStaf}`);
            return result.data
        }
    }
                            ...
                            {filteredOption && filteredOption.length === 0 && query !== '' ? (
                                ...
                                filteredOption && filteredOption.map((permohonan: Permohonan) => (
                                    ...
                                    <Combobox.Option
                                        key={permohonan.id}
                                        className={({ active }) =>
                                            `relative cursor-default select-none pr-2 ${active ? 'bg-lightBlue-600 text-white' : 'text-lightBlue-950'
                                            }`
                                        }
                                        value={permohonan}
                                    >
