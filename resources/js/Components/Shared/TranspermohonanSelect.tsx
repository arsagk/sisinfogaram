...
import { OptionSelect, Permohonan, TPermohonan, Transpermohonan } from '@/types'
...
type Props = {
    className?: string,
    value?: Transpermohonan | undefined,
    errors?: string,
    onValueChange: (e: Transpermohonan, opt: OptionSelect) => void
}
export default function TranspermohonanSelect({ className, onValueChange, value, errors }: Props) {
    const [selectedPerson, setSelectedPerson] = useState<Transpermohonan | undefined>(value ? value : undefined,);
    ...
    async function fetcher(url: string, query: string | unknown): Promise<Transpermohonan[]> {
        if (searchKey) {
    ...
    const { data: filteredOption, error } = useSWR<Transpermohonan[]>(['/admin/transpermohonans/api/list/', query], ([url, query]) => fetcher(url, query));
    ...
    const comparePeople = (a?: Transpermohonan, b?: Transpermohonan): boolean =>
        a?.permohonan?.nama_penerima.toLowerCase() === b?.permohonan?.nama_penerima.toLowerCase();
                ...
                <div className={twMerge(`relative z-50`, className)}>
                    <div className={twMerge("relative overflow-visible flex w-full cursor-default bg-white text-left rounded-lg shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm", className)} >
                        ...
                        <Combobox.Input
                            ...
                            displayValue={(transpermohonan: Transpermohonan) => transpermohonan.permohonan?.nama_penerima ?? ''}
                    ...
                    >
                        <Combobox.Options className="absolute mt-1 max-h-60 w-full md:w-[100vh] overflow-auto rounded-md bg-white py-0 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-xs">
                            ...
                            {filteredOption && filteredOption.length === 0 && query !== '' ? (
                                ...
                                filteredOption && filteredOption.map((transpermohonan: Transpermohonan) => (
                                    ...
                                    >
                                        {({ selected, active }) => (
                                            <div className={`flex flex-row p-2 w-full ${selected ? 'font-bold' : ''}`}>
                                                ...
                                                <div className='w-1/3'>
                                                    {transpermohonan.permohonan?.no_daftar}
                                                </div>
                                                <div className='w-3/5'>
                                                ...
                                                <div className='w-60'>
                                                    {transpermohonan.permohonan?.luas_tanah} m2
                                                </div>
