...
const CardDkeluarbiayapermuserList = ({ keluarbiayapermuser }: { keluarbiayapermuser: Keluarbiayapermuser }) => {
    type Props = {
    ...
    const { dkeluarbiayapermusers: { data, links, meta } } = usePage<Props>().props;
                ...
                {data && data.map((item, index: number) => (
                    <li key={item.id} className='w-full flex flex-col overflow-hidden bg-lightBlue-300'>
                        <div className='flex w-full gap-1 text-xs px-2 py-0 items-center justify-start text-lightBlue-900 border-b-2 border-lightBlue-200'>
                            ...
                            <div className='pb-1 w-[35%]'>{item.permohonan}</div>
                            <div className='pb-1 w-[35%] md:w-[20%]'>{item.nama_itemkegiatan}</div>
                            <div className='pb-1 hidden md:block md:w-[20%]'>{item.ket_biaya}</div>
                            ...
                            <div className='pb-1 w-[5%] flex justify-center items-center gap-2'>
                                <button disabled={keluarbiayapermuser.status_keluarbiayapermuser == 'approved'} onClick={(e) => useSwal.confirm({
            ...
            <ul className='list-none container-snap max-h-80 overflow-x-hidden rounded-b-md shadow-md'>
                <li className='w-full flex flex-col overflow-hidden bg-lightBlue-600 px-2 py-1  font-semibold text-lightBlue-50'>
                    <div className='flex w-full gap-1 text-xs px-2 py-1 items-center justify-start border-b-2 border-lightBlue-500'>
                        ...
                        <div className='w-[15%] text-right'>{keluarbiayapermuser.jumlah_biaya}</div>
                    ...
                    <div className='flex w-full gap-1 text-xs px-2 py-1 items-center justify-start border-b-2 border-lightBlue-500'>
                        ...
                        <div className='w-[15%] text-right'>{keluarbiayapermuser.saldo_awal}</div>
                        <div className='w-[5%] flex justify-start items-center gap-2'>
                        </div>
                    </div>
                    <div className='flex w-full gap-1 text-xs px-2 py-1 items-center justify-start border-b-2 border-lightBlue-500'>
                        ...
                        <div className='w-[15%] text-right'>{keluarbiayapermuser.saldo_akhir}</div>
                        <div className='w-[5%] flex justify-start items-center gap-2'>
                        </div>
                    </div>
                </li>
