...
import { Keluarbiaya, Keluarbiayapermuser, OptionSelect } from "@/types";
...
// components
export default function CardTableKeluarbiayas({ color = "light", keluarbiayas, className = "", meta, labelLinks }: { color: "dark" | "light", keluarbiayas: Keluarbiaya[], className?: string, meta: { links: [], per_page: number, total: number }, labelLinks: any }) {
    ...
    const { isAdmin, user, base_route } = usePage<{ isAdmin: boolean, user: OptionSelect, base_route: string }>().props
    ...
    //     router.delete(route('transaksi.keluarbiayas.destroy', id))
    // }
            ...
            >
                ...
                <div className="block w-full overflow-x-auto">
                    <table className="items-center w-full bg-transparent border-collapse">
                        ...
                        <tbody>
                            {
                                keluarbiayas.map(({ id, created_at, instansi, metodebayar, kasbons, user, status_keluarbiaya }, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-1">
                                            {kasbons.length > 0 ? kasbons.map((e, i) => (
                                                <span key={i}>{e.id}</span>
                                            )) : (<span>No Kasbon</span>)}
                                        </td>
                                        <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-1">
                                            {user.name}
                                        ...
                                        <td className="border-t-0 px-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-1 ">
                                            <Link
                                                ...
                                                className="text-lightBlue-500 background-transparent font-bold uppercase px-2 py-1 text-sm outline-none focus:outline-none ease-linear transition-all duration-150" type="button">
