...
import { Postingjurnal } from "@/types";
...
// components
export default function CardTablePostingjurnals({ color = "light", postingjurnals, className = "", meta, labelLinks }: { color: "dark" | "light", postingjurnals: Postingjurnal[], className?: string, meta: { links: [], per_page: number, total: number }, labelLinks: any }) {
                    ...
                    <table className="items-center w-full bg-transparent border-collapse">
                        <thead>
                            <tr>
                                ...
                                >
                                    <Link href="#" onClick={(e) => handleSortLinkClick({ sortBy: 'uraian', sortDir: values.sortDir === 'asc' ? 'desc' : 'asc' })} >
                                        <div className="flex flex-row justify-between">
                                            <span>Uraian</span>
                                            <IconSort sortBy='uraian' sortDir={values.sortDir || ''} />
                                        </div>
                                    </Link>
                                </th>
                                <th
                        ...
                        <tbody>
                            {
                                postingjurnals.map(({ id, uraian, user, jumlah }, index) => (
                                    <tr key={index}>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                                            {id}
                                        </td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                                            <i className="fas fa-circle text-orange-500 mr-2"></i> {uraian}
                                        </td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                                            {jumlah}
                                        </td>
                                        ...
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2 ">
                                            <div className="flex justify-start gap-1 ">
                                                <Link href={route('admin.transaksi.postingjurnals.edit', id)} className="text-lightBlue-500 background-transparent font-bold px-3 py-1 text-xs outline-none focus:outline-none hover:text-lightBlue-100 hover:scale-105 mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                                                    Edit
                                                </Link>
                                                <button
