import { OptionSelect, Biayaperm, OptionSelectDisabled, KeluarBiayaperm, Transpermohonan, Dkeluarbiayapermuser, DkeluarbiayapermuserStaf } from '@/types'
import React, { useEffect, useState } from 'react'
import apputils from '@/bootstrap'
import { router } from '@inertiajs/react';
import useSwal from '@/utils/useSwal';
import { ChevronDoubleLeftIcon, ChevronLeftIcon, ChevronRightIcon, TrashIcon } from '@heroicons/react/20/solid';
...
function CardTableKeluarbiayaperms({ transpermohonan }: Props) {
    ...
    const [keluarbiayaperms, setKeluarbiayaperms] = useState<KeluarBiayaperm[]>()
    const [keluarbiayapermusers, setKeluarbiayapermusers] = useState<DkeluarbiayapermuserStaf[]>()
    ...
    const getKeluarbiayaperms = async (transpermohonan_id: string, link = null) => {
        setIsloading(true)
    ...
    const getKeluarbiayapermusers = async (transpermohonan_id: string, link = null) => {
        setIsloading(true)
        ...
        <div className="w-full px-1 py-1 sm:px-0">
            <Tab.Group>
                ...
                <Tab.Panels className="mt-2">
                            ...
                            >
                                ...
                                <div className="w-full mt-2 flext flex-col">
                                    <div className='flex flex-row w-full px-4 justify-center items-center rounded-t-md text-xs bg-lightBlue-600 border-blueGray-400 py-2  text-lightBlue-50 font-semibold'>
                                        ...
                                        <div className='w-[35%] md:w-[15%]'>Metode</div>
                                        <div className='w-[35%] md:w-[20%]'>Item Kegiatan</div>
                                        <div className='hidden md:block w-[10%]'>Catatan</div>
                                        <div className='hidden md:flex md:w-[15%]'>User</div>
                                        <div className='w-[35%] md:w-[15%]'>Jumlah</div>
                                        <div className='flex md:w-[10%]'>Menu</div>
                                    </div>
                                    <ul className='list-none container-snap max-h-80 overflow-x-hidden rounded-b-md'>
                                        ...
                                        {keluarbiayaperms && keluarbiayaperms.map((keluarbiayaperm: KeluarBiayaperm, index: number) => (
                                            <li key={keluarbiayaperm.id} className='w-full flex flex-col overflow-hidden bg-lightBlue-100 px-4 border-b-2 border-lightBlue-200'>
                                                <div className='flex text-xs py-1 items-center justify-center font-semibold text-lightBlue-600 '>
                                                    <div className='w-[5%] '>{index + 1}</div>
                                                    <div className='w-[10%]'>{keluarbiayaperm.tgl_keluarbiayaperm}</div>
                                                    <div className='w-[35%] md:w-[15%]'>{keluarbiayaperm.metodebayar.nama_metodebayar}</div>
                                                    <div className='w-[35%] md:w-[20%]'>{keluarbiayaperm.itemkegiatan.nama_itemkegiatan}</div>
                                                    <div className='hidden md:block md:w-[10%]'>{keluarbiayaperm.catatan_keluarbiayaperm}</div>
                                                    <div className='hidden md:flex md:w-[15%]'><span>{keluarbiayaperm.user.name}</span></div>
                                                    <div className='w-[35%] md:w-[15%]'>{keluarbiayaperm.jumlah_keluarbiayaperm}</div>
                                                    <div className='flex md:w-[10%]'>
                                                        {/* <button
                                                            ...
                                                            className="text-lightBlue-500 background-transparent font-bold px-3 py-0 text-xs outline-none focus:outline-none hover:text-lightBlue-100 hover:scale-105 mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                                                            <TrashIcon className="m-auto h-4 w-4" stroke='#58c4fa' fill='none' strokeWidth={2} />
                                                        </button> */}
                                                    </div>
                                                </div>
                                            </li >))}
                                ...
                                <div className='flex justify-end items-center gap-2 mt-2'>
                                    ...
                                    <div className='w-[79%] flex justify-between text-xs font-bold gap-2 text-blueGray-500'>
                                        <span className='flex justify-start items-center'>
                                            {/* <LinkButton href='#' theme='black' className='shrink-0 py-1 mt-0 mb-0 text-xs' onClick={(e) => {
                                                e.preventDefault()
                                        ...
                                        {totalPengeluaran ?
                                            <span className='flex items-center'>Total : {totalPengeluaran}</span>
                                            : null}
                                    </div>
                                    <div className='w-[30%] md:w-[21%] flex justify-end items-center'>
                                        {prevLink ?
                                            <button
                                                className="text-lightBlue-500 background-transparent font-bold uppercase px-2 py-1 text-sm outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button"
                                                onClick={(e) => getKeluarbiayaperms(transpermohonan.id, prevLink)}
                                            >
                                                <i className="fa fa-chevron-left" aria-hidden="true"></i>
                                            </button> : <span className="text-blueGray-500 background-transparent font-bold uppercase px-2 py-1 text-sm outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"><i className="fa fa-chevron-left" aria-hidden="true"></i>
                                            </span>}
                                        {nextLink ?
                                            <button
                                                className="text-lightBlue-500 background-transparent font-bold uppercase px-2 py-1 text-sm outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" type="button"
                                            ...
                                            </button> : <span className="text-blueGray-500 background-transparent font-bold uppercase px-2 py-1 text-sm outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"><i className="fa fa-chevron-right" aria-hidden="true"></i>
                                            </span>}
                                    </div>
                                </div>
                                {/* end pengeluaran */}
                            ...
                            >
                                ...
                                <div className="w-full mt-2 flext flex-col">
                                    <div className='flex flex-row w-full px-4 justify-center items-center rounded-t-md text-xs bg-lightBlue-600 border-blueGray-400 py-2  text-lightBlue-50 font-semibold'>
                                        ...
                                        <div className='w-[35%] md:w-[13%]'>Instansi</div>
                                        <div className='w-[35%] md:w-[10%]'>Metode</div>
                                        <div className='w-[35%] md:w-[15%]'>Item Kegiatan</div>
                                        <div className='md:block w-[15%]'>Jumlah Biaya</div>
                                        <div className='hidden md:flex md:w-[15%]'>Ket Biaya</div>
                                        <div className='w-[35%] md:w-[12%]'>User</div>
                                    </div>
                                    <ul className='list-none container-snap max-h-80 overflow-x-hidden rounded-b-md'>
                                        {isloading &&
                                            <div className='m-auto h-24 w-full flex justify-center items-center absolute'>
                                        ...
                                        {keluarbiayapermusers && keluarbiayapermusers.map((dkeluarbiayapermuser: DkeluarbiayapermuserStaf, index: number) => (
                                            <li key={dkeluarbiayapermuser.id} className='w-full flex flex-col overflow-hidden bg-lightBlue-100 px-4 border-b-2 border-lightBlue-200'>
                                                <div className='flex text-xs py-1 items-center justify-center font-semibold text-lightBlue-600 '>
                                                    <div className='w-[5%] '>{index + 1}</div>
                                                    <div className='w-[15%]'>{dkeluarbiayapermuser.created_at}</div>
                                                    <div className='w-[35% md:w-[13%]'>{dkeluarbiayapermuser.instansi.nama_instansi}</div>
                                                    <div className='w-[35%] md:w-[10%]'>{dkeluarbiayapermuser.metodebayar.nama_metodebayar}</div>
                                                    <div className='w-[35%] md:w-[15%]'>{dkeluarbiayapermuser.itemkegiatan.nama_itemkegiatan}</div>
                                                    <div className='w-[35%] md:w-[15%]'>{dkeluarbiayapermuser.jumlah_biaya}</div>
                                                    <div className='md:block md:w-[15%]'>{dkeluarbiayapermuser.ket_biaya}</div>
                                                    <div className='hidden md:flex md:w-[12%]'><span>{dkeluarbiayapermuser.user.name}</span></div>
                                                </div>
                                            </li >))}
                                ...
                                <div className='flex justify-end items-center gap-2 mt-2'>
                                    ...
                                    <div className='w-[79%] flex justify-between text-xs font-bold gap-2 text-blueGray-500'>
                                        ...
                                        {totalPengeluaran ?
                                            <span className='flex items-center'>Total : {totalPengeluaranUser}</span>
                                            : null}
                                    </div>
                                    <div className='w-[30%] md:w-[21%] flex justify-end items-center'>
                                        {prevLinkUser ?
                    ...
                    >
                        <SubCardNeracaTableKeluarBiayaPermUser neracas={neracas} totDebet={totDebet} totKredit={totKredit} />
                    </Tab.Panel>
                </Tab.Panels>
            </Tab.Group>
            {
                transpermohonan &&
                <>
                    <ModalKeluarBiayaperm transpermohonan={transpermohonan} showModal={showModalKeluarbiayaperm} setShowModal={setShowModalKeluarbiayaperm} />
                </>
            }
        </div >
    )
