...
const ModalBayarbiayaperms = ({ showModal, setShowModal, biayaperm_id }: Props) => {
        ...
        <Modal show={showModal} maxWidth='2xl' closeable={true} onClose={() => setShowModal(false)}>
            <div className='p-4 bg-blueGray-100 rounded-md text-xs'>
                <div className='w-full absolute right-1 top-1 flex justify-between items-center px-1'>
                    <h1 className='text-lg mb-1 font-semibold text-blueGray-500 ml-4'>List Pembayaran</h1>
                ...
                <table className='w-full shadow-md bottom-2 mt-5'>
                    <thead>
                        <tr className='border-y-2 font-semibold bg-slate-300'>
                            <td className='px-1 py-2' width="5%">No</td>
                            <td className='px-1 py-2' width="15%">Tanggal</td>
                            <td className='px-1 py-2' width="15%">Metode Bayar</td>
                            <td className='px-1 py-2' width="20%">Info Rekening</td>
                            <td className='px-1 py-2' width="15%" align='right'>Saldo Awal</td>
                            <td className='px-1 py-2' width="15%" align='right'>Jumlah Bayar</td>
                            <td className='px-1 py-2' width="15%" align='right'>Saldo Akhir</td>
                        </tr>
                    </thead>
                    <tbody>
                        {bayarbiayaperms && bayarbiayaperms.map((bayarbiayaperm, idx) => (
                            <tr key={bayarbiayaperm.id}>
                                <td className='px-1 py-2'>{idx + 1}</td>
                                <td className='px-1 py-2'>{bayarbiayaperm.tgl_bayarbiayaperm}</td>
                                <td className='px-1 py-2'>{bayarbiayaperm.metodebayar.nama_metodebayar}</td>
                                <td className='px-1 py-2'>{bayarbiayaperm.info_rekening}</td>
                                <td className='px-1 py-2' align='right'>{bayarbiayaperm.saldo_awal}</td>
                                <td className='px-1 py-2' align='right'>{bayarbiayaperm.jumlah_bayar}</td>
                                <td className='px-1 py-2' align='right'>{bayarbiayaperm.saldo_akhir}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
