...
const ModalBayarBiayaperm = ({ showModal, setShowModal, biayaperm }: Props) => {
    const { metodebayars } = usePage<{ metodebayars: OptionSelect[] }>().props;
        ...
        <Modal show={showModal} maxWidth='md' closeable={false} onClose={() => setShowModal(false)}>
            <div className='p-4 bg-blueGray-100 rounded-md'>
                <form onSubmit={handleSubmit}>
                    <h1 className='font-bold text-xl text-blueGray-700 mb-4'>BAYAR BIAYA PERMOHONAN</h1>
                    <MoneyInput name='saldo_awal' label='Jumlah Biaya' errors={errors.saldo_awal}
                        value={data.saldo_awal}
                        disabled
                        onValueChange={e => setData(prev => ({
                            ...prev, 'saldo_awal': e.value,
                    ...
                    <Input name='info_rekening' label='Info Pembayaran' errors={errors.info_rekening} value={data.info_rekening} onChange={e => setData('info_rekening', e.target.value)} />

                    <MoneyInput name='jumlah_bayar' label='Jumlah Bayar' errors={errors.jumlah_bayar}
                    ...
                    <MoneyInput name='saldo_akhir' label='Kurang Bayar' disabled errors={errors.saldo_akhir} value={data.saldo_akhir} onValueChange={e => {
                        setData('saldo_akhir', e.value)
                    }} />
                    <Input name='catatan_biayaperm' label='Catatan' errors={errors.catatan_bayarbiayaperm} value={data.catatan_bayarbiayaperm} onChange={e => setData('catatan_bayarbiayaperm', e.target.value)} />
