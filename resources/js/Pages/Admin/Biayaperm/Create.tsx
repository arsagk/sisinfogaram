...
import CardTableBiayaperms from '@/Components/Cards/Admin/CardTableBiayaperms';
import CardTableKeluarbiayaperms from '@/Components/Cards/Admin/CardTableKeluarbiayaperms';
...
import { Biayaperm, Itemprosesperm, OptionSelect, OptionSelectDisabled, Permohonan, Prosespermohonan, Statusprosesperm, Transpermohonan } from '@/types';
...
const Create = () => {
    const { permohonan, biayaperms, transpermohonan, transpermohonanopt, transpermohonans, permohonan_id, transpermohonan_id } = usePage<{
        permohonan: Permohonan,
        biayaperms: Biayaperm[],
        transpermohonan: Transpermohonan,
        transpermohonanopt: OptionSelect,
        transpermohonans: Transpermohonan[],
        permohonan_id: string, transpermohonan_id: string
    }>().props;
    type FormValues = {
    ...
    const setPermohonan = (permohonan: Permohonan) => {
        if (permohonan) {
            ...
            setValues(prev => ({ ...prev, permohonan_id: permohonan.id, transpermohonan_id: permohonan?.transpermohonan.id }))
        }
    ...
    const [values, setValues] = useState({ permohonan_id: permohonan_id, transpermohonan_id: transpermohonan_id });
    ...
    return (
        <AdminLayout>
                    ...
                    <div className="relative flex flex-col min-w-0 break-words w-full mb-2 shadow-lg shadow-slate-400 rounded-lg bg-blueGray-200 border-0">
                        ...
                        <div className="flex-auto px-4 py-2">
                            ...
                            <div className='flex flex-col md:flex-row items-center w-full gap-1'>
                                <PermohonanSelect isStaf={false} className={`w-full ${AddMode ? 'z-10' : ''}`} value={permohonan} onValueChange={setPermohonan} errors={errors.permohonan_id} />
                            </div>
                            <Suspense fallback={
                                ...
                                <div>
                                    {permohonan ? <CardPermohonan permohonan={permohonan} /> : null}
                                </div>
                            </Suspense>
                            ...
                            {permohonan ?
                                <div className='bg-blueGray-300 rounded-md shadow-md py-2 px-2 flex justify-between items-center'>
                                    <div className='w-1/2 md:w-1/3'>
                                        <SelectSearch options={transpermohonans} value={transpermohonanopt}
                                            onChange={(e: any) => setValues((prev) => ({ ...prev, transpermohonan_id: e ? e.value : '' }))} className='mb-0' placeholder="Pilih Jenis Permohonan" />
                                    </div>
                            ...
                            {transpermohonan &&
                                <>
                                    <CardTableKeluarbiayaperms transpermohonan={transpermohonan} />
                                </>
                            }
                        </div>
                    </div>
            ...
            <ModalAddBiayaperm showModal={showModalBiayaperm} setShowModal={setShowModalBiayaperm} />
